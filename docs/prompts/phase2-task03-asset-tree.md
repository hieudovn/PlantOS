# Phase 2 — Task 2-03: Asset Tree View

> **Designer:** DeepSeek V4 Pro | **Date:** 2026-06-30

## Context

Thay Asset Table bằng Tree View hiển thị UNS hierarchy: Plant → Area → Asset(parent→child). Giữ search/filter + table view toggle.

Frontend-only (1-2 files). API đã có đủ data.

## Implementation Checklist

- [ ] CREATE `frontend/src/features/assets/AssetTree.tsx` — tree component
- [ ] MODIFY `frontend/src/features/assets/AssetTable.tsx` — add tree/table toggle

## Detailed Instructions

### 1. `AssetTree.tsx` — Tree Component

Fetch assets + areas → build hierarchy tree → render expandable nodes.
...
import { ChevronRight, ChevronDown, Factory, MapPin, Wrench } from "lucide-react";
  ...
  return (
    <div>
      ...
      >
        ...
        {node.assetId && <span className="font-mono text-xs text-gray-500">{node.assetId}</span>}
...
### 2. `AssetTable.tsx` — Add Tree/Table Toggle

Thêm toggle button ở đầu AssetTable:
...
// In JSX, before the table/tree:
<div className="flex items-center gap-2 mb-3">
  <button onClick={() => setViewMode("tree")}
    className={`px-3 py-1 text-xs rounded ${viewMode === "tree" ? "bg-gray-700 text-white" : "text-gray-500"}`}>
    🌳 Tree
  </button>
  <button onClick={() => setViewMode("table")}
    className={`px-3 py-1 text-xs rounded ${viewMode === "table" ? "bg-gray-700 text-white" : "text-gray-500"}`}>
    📋 Table
  </button>
</div>

// Conditional render:
{viewMode === "tree" ? <AssetTree /> : (/* existing table */)}
```
...
# Verify: tree view with hierarchy, expand/collapse, click → detail
# Toggle to table view → verify filters still work
```
...
| # | File | Action |
|---|------|--------|
| 1 | `frontend/src/features/assets/AssetTree.tsx` | CREATE |
| 2 | `frontend/src/features/assets/AssetTable.tsx` | MODIFY |
